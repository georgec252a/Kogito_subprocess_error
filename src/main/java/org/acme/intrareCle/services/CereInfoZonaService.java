...
public class CereInfoZonaService {
    ...
    public IntrareCLE cereInfo(IntrareCLE intrareCle, String failService) {
        ...
        fail = Optional.ofNullable(CereInfoZonaService.class)
                .map(Class::getSimpleName)
                .map(n -> Objects.equals(failService, n))
                .orElse(false);

        if (fail)

            throw new ServiceException("eroare");
        else
            return intrareCle;
