...
@Service
public class ZonaLocuriLibereService {
    public final String STARE_FABRICATIE = "0";

    private static final Logger logger = LoggerFactory.getLogger(ZonaLocuriLibereService.class);

    public IntrareCLE cautaAltaZona(IntrareCLE intrareCle) {
        ...
        logger.info("Se cauta o alta zona cu locuri libere");

        intrareCle.setStocareCLE(true);
        intrareCle.setLocuriLibere(2);
        intrareCle.setLocuriLibereOk(true);

        logger.info("Aici se introduce loc pana la gasirea unei zone cu cel putin un loc liber");
